...
class PseudoDataset(ImageFolder):
    def __init__(self, root: str, configs, train, 
        ...
        self.interventions = [
            transforms.ColorJitter(
                brightness=configs.colorjitter_brightness,
                contrast=configs.colorjitter_contrast,
                saturation=configs.colorjitter_saturation,
                hue=configs.colorjitter_hue
            ),
            transforms.Grayscale(
                num_output_channels=configs.grayscale_num_output_channels
            ),
            transforms.GaussianBlur(
                kernel_size=configs.gaussianblur_kernel_size,
                sigma=(configs.gaussianblur_sigma_min, configs.gaussianblur_sigma_max)
            ),
            transforms.RandomInvert(
                p=configs.randominvert_p
            ),
            transforms.RandomPosterize(
                bits=configs.randomposterize_bits,
                p=configs.randomposterize_p
            ),
            transforms.RandomSolarize(
                threshold=configs.randomsolarize_threshold,
                p=configs.randomsolarize_p
            ),
            transforms.RandomEqualize(
                p=configs.randomequalize_p
            )
        ]
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            aug_index = random.randint(0, len(self.interventions)-1)
            intervention = self.interventions[aug_index - 1]
            aug = transforms.Compose([intervention, self.transform])
            x_aug = aug(sample)
            x_orig = self.transform(sample)
            return (x_orig, x_aug, path), target
        else:
...
class MultipleEnvironmentImageFolder(MultipleDomainDataset):
    def __init__(self, root, test_envs, augment, hparams):
        ...
        for i, environment in enumerate(environments):
            ...
            env_dataset = PseudoDataset(path,
                transform=env_transform, configs=hparams['configs'], train=(i not in test_envs),)
